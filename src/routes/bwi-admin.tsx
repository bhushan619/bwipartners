...
function BwiAdmin() {
  ...
  const submit = () => {
    ...
    setRates((current) => current.map((row, index) => index === 3 ? [row[0], form.currency || "[CCY]"] : row));
    setSaved(true);
    setView("rates");
  };
